...
import { options as _options, Component } from 'preact';
...
interface ComponentWithHooks extends Component<any, any> {
	__hooks?: {
		_list: Array<{ _args?: any[] }>;
	};
	_childDidSuspend?: boolean;
}

function getDomChildren(vnode: VNode): string[] {
	let domChildren: string[] = [];

	if (!vnode._children) return domChildren;

	vnode._children.forEach((child: any) => {
...
function getClosestDomNodeParentName(parent: VNode | null): string {
	if (!parent) return '';
	if (typeof parent.type == 'function') {
		if (parent._parent == null) {
			if (parent._dom != null && (parent._dom as any).parentNode != null) {
				return (parent._dom as any).parentNode.localName;
			}
			return '';
		}
		return getClosestDomNodeParentName(parent._parent);
	}
	return parent.type as string;
}
...
export function initDebug(): void {
	...
	options._catchError = (
		...
		if (component && typeof error.then == 'function') {
			...
			for (; parent; parent = parent._parent) {
				if (
					parent._component &&
					(parent._component as ComponentWithHooks)._childDidSuspend
				) {
					error = promise;
					break;
				}
	...
	options._root = (vnode: any, parentNode: any) => {
		if (!parentNode) {
			throw new Error(
	...
	options._diff = (vnode: any) => {
		...
		if (typeof vnode.type == 'function' && (vnode.type as any).propTypes) {
			if (
				(vnode.type as any).displayName === 'Lazy' &&
				warnedComponents &&
				...
				try {
					const lazyVNode = (vnode.type as any)();
	...
	options._hook = (comp: any, index: number, type: number) => {
	...
	options.diffed = (vnode: any) => {
		...
		if (
			...
			(vnode._component as ComponentWithHooks).__hooks != null
		) {
			...
			const hooks = (vnode._component as ComponentWithHooks).__hooks!._list;
			if (hooks) {
...
Component.prototype.forceUpdate = function (this: any, callback?: () => void) {
	if (this._vnode == null) {
		console.warn(
...
export function serializeVNode(vnode: VNode): string {
	...
	for (let prop in props as any) {
		if ((props as any).hasOwnProperty(prop) && prop !== 'children') {
...
options._hydrationMismatch = (newVNode: any, excessDomChildren: any) => {
	const { type } = newVNode;
	const availableTypes = excessDomChildren
		.map((child: any) => child && child.localName)
		.filter(Boolean);
	console.error(

...
const onChangeInputType = (type: string): boolean => /fil|che|rad/.test(type);
...
[
	...
	Object.defineProperty(Component.prototype, key, {
		configurable: true,
		get() {
			return (this as any)['UNSAFE_' + key];
		},
		set(v: any) {
...
export function render(
	vnode: VNode,
	parent: PreactElement,
	callback?: () => void
): CompatComponent | null {
	// React destroys any existing DOM nodes, see #1727
	// ...but only on the first render, see #1828
	if ((parent as any)._children == null) {
		(parent as any).textContent = '';
	}

	preactRender(vnode as any, parent as any);
	if (typeof callback == 'function') callback();

	return vnode ? (vnode as any)._component : null;
}
...
export function hydrate(
	...
	preactHydrate(vnode as any, parent as any);
	if (typeof callback == 'function') callback();
...
function isPropagationStopped(this: any): boolean {
	return this.cancelBubble;
}

function isDefaultPrevented(this: any): boolean {
...
function handleDomVNode(vnode: VNode): void {
	let props = vnode.props as Record<string, any>,
	...
	if (
		...
		normalizedProps.value = toChildArray(props.children).forEach(
			(child: any) => {
...
let currentComponent: CompatComponent | null;
const oldBeforeRender = options._render;
options._render = function (vnode: any) {
	if (oldBeforeRender) {
...
options.diffed = function (vnode: any) {
	if (oldDiffed) {
		oldDiffed(vnode);
	}
	...
	ReactCurrentDispatcher: {
		current: {
			readContext(context: any) {
				return (currentComponent as any)._globalContext[context._id].props
					.value;
			},
			useCallback,
			useContext,
